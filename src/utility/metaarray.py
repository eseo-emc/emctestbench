...
#TODO: think up needed functionality (multi-dimensional, non-contiguous slices) and write corresponding tests
#TODO: make Axis class
#TODO: test and fix writout and readin of proprietary format
#TODO: test and write writeout and readin of XML
#TODO: create and use Unit class, that can render to plaintext and LaTeX
	
from numpy import ndarray, array, empty, fromstring, arange
import types, copy


def axis(name=None, cols=None, values=None, units=None, title=None):
...
class MetaArray(ndarray):
	...
	def _labelFromInfo(self,info):
		"""Return a pretty label, given meta-info (with keys like name, title and units"""
		labelString = '(unknown)'
		if info.has_key('title'):
			labelString = info['title']
		elif info.has_key('name'):
			labelString = info['name']
		if info.has_key('units'):
			labelString = '%s (%s)' % (labelString,info['units'])
		return labelString
	
	def axisLabel(self, axis):
		"""Return a pretty label for the axis"""
		ax = self._info[self._interpretAxis(axis)]
		return self._labelFromInfo(ax)
	...
	def columnLabel(self, axis, column):
		"""Return a pretty label for column in axis"""
		ax = self._info[self._interpretAxis(axis)]
		if ax.has_key('cols'):
			for c in ax['cols']:
				if c['name'] == column:
					return self._labelFromInfo(c)
			raise Exception("Axis %s has no column named %s" % (str(axis), str(column)))
		else:
			raise Exception("Axis %s has no column definitions" % str(axis))
	...
	def writeToExcel(self,workbookPath):
		'''
		Flatten the MetaArray and write to Excel file. Currently, only this template is supported:
		axis 0: Independent variable, will be converted to rows
		axis 1: One or more dependent variables (always as cols), will be converted to columns
		axis 2 (optional): Trial, will be converted to worksheets.
		'''
		import xlwt, xlrd
		newWorkbook = xlwt.Workbook()
		
		def writeSheet(sheetName,metaArray):
			newSheet = newWorkbook.add_sheet(sheetName)		
			# Process first axis
			def writeArray(startRow,startColumn,values):
				'''Write array values top-down, starting in startRow,startColumn'''
				for index,value in enumerate(values):
					newSheet.write(index+startRow,startColumn,value) 
			 
			newSheet.write(0,0,metaArray._info[0]['name'])
			newSheet.write(1,0,metaArray.axisLabel(0))
			writeArray(2,0,metaArray.axisValues(0))
			
			# Process second axis
			secondAxis = metaArray._info[1]
			if secondAxis.has_key('cols'):
				for columnIndex,columnInfo in enumerate(secondAxis['cols']):
					newSheet.write(0,columnIndex+1,columnInfo['name'])
					newSheet.write(1,columnIndex+1,self._labelFromInfo(columnInfo))
					writeArray(2,columnIndex+1,metaArray[:,columnIndex].view(ndarray))
			else:
				raise NotImplementedError('Axis 1 must have columns')
	
		if len(self._info) == 4:
			if self._info[2].has_key('cols'):
				for columnIndex,columnInfo in enumerate(self._info[2]['cols']):
					writeSheet(columnInfo['name'],self[:,:,columnIndex])
			else:
				raise NotImplementedError('Axis 2 must have columns')
		elif len(self._info) == 3:
			writeSheet('Data',self)
		else:
			raise NotImplementedError('The MetaArray must have 2 or 3 axes for Excel export and has %d axis' % len(self._info)-1)
	
		newWorkbook.save(workbookPath)
...
#			axis('t', values=[0,1,2,3], units='s',title='Time'), 
#			axis('Signal', cols=[('V0', 'V', 'Voltage 0'),('V1', 'V', 'Voltage 1')]),
#			axis('trial', cols=[('No interference'),('Interference')])
#		])
#	demoArray2['Signal':'V0','trial':'No interference'] = [1,3,2,4]
#	demoArray2['Signal':'V1','trial':'No interference'] = [7,6,5,4]
#	demoArray2['Signal':'V0','trial':'Interference'] = [2,3,2,4]
#	demoArray2['Signal':'V1','trial':'Interference'] = [9,6,5,4]
#	
#	#print demoArray
#	
